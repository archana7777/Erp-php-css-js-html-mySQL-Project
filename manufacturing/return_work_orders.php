...
***********************************************************************/
$page_security = 'SA_RETURNWORKORDERENTRY';
$path_to_root = "..";
...
// curently outstanding simply means not closed
	$outstanding_only = 1;
	page(_($help_context = "Return Work Orders (WM-TM)"), false, false, "", $js);
}
else
{
	$outstanding_only = 0;
...
function edit_link($row)
{
	return  $row['closed'] ? '<i>'._('Closed').'</i>' :
		$row['approved'] ? '<i>'._('Approved').'</i>' :pager_link(_('Update Status'),
			"/manufacturing/update_wm_tm_work_order.php?trans_no=" .$row["id"]);
}
...
$sql = get_sql_for_return_work_orders($outstanding_only, get_post('SelectedStockItem'), get_post('StockLocation'),
