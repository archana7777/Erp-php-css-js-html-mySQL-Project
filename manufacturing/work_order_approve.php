...
page(_($help_context = "Work Order Approve to Manufacturing"), false, false, "", $js);
...
{
	approve_work_order($selected_id, $_POST['approved_date'], $_POST['memo_']);

	display_notification(_("The work order has been approved to manufacturing."));
...
if (can_process($myrow))
{
    ...
    date_row(_("Approved Date") . ":", 'approved_date');

    textarea_row(_("Memo:"), 'memo_', $_POST['memo_'], 40, 5);

    end_table(1);

    submit_center('approve', _("Approve Work Order"), true, '', 'default');
