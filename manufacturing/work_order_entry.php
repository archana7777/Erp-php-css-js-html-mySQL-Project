...
page(_($help_context = "Work Order Entry (TM-WM)"), false, false, "", $js);
...
if (isset($_POST['ADD_ITEM']) && can_process())
{
	...
	$id = add_work_order($_POST['wo_ref'], $_POST['StockLocation'], input_num('quantity'),
		...
		$_POST['RequDate'], $_POST['memo_'], input_num('Costs'), $_POST['cr_acc'], input_num('Labour'), $_POST['cr_lab_acc'],input_num('qty_per_day'),$_POST['ExeDate']);
...
if (isset($_POST['UPDATE_ITEM']) && can_process())
{
	...
	update_work_order($selected_id, $_POST['StockLocation'], input_num('quantity'),
		$_POST['stock_id'],  $_POST['date_'], $_POST['RequDate'], $_POST['memo_'],input_num('qty_per_day'),$_POST['ExeDate']);
	new_doc_date($_POST['date_']);
...
if (isset($selected_id))
{
	...
	$_POST['qty_per_day'] = $myrow["units_reqs_per_day"];
	...
	$_POST['ExeDate'] = sql2date($myrow["execute_by"]);
...
else
{
	...
	label_row(_("Size:"),$item_info['size']);
	label_row(_("Grade:"),$item_info['grade']);
	label_row(_("Reference Specification:"),$item_info['standard']);
}

if (!isset($_POST['quantity']))
{
	$_POST['quantity'] = qty_format(1, $_POST['stock_id'], $dec);
	$_POST['qty_per_day'] = qty_format(1, $_POST['stock_id'], $dec);	
}
else
{
	$_POST['quantity'] = qty_format($_POST['quantity'], $_POST['stock_id'], $dec);
	$_POST['qty_per_day'] = qty_format($_POST['qty_per_day'], $_POST['stock_id'], $dec);		
}
	

if (get_post('type') == WO_ADVANCED)
{
    qty_row(_("Quantity to Produce in MT:"), 'quantity', null, null, null, $dec);
    if ($_POST['released'])
    	label_row(_("Quantity Manufactured:"), number_format($_POST['units_issued'], get_qty_dec($_POST['stock_id'])));
    date_row(_("Date") . ":", 'date_', '', true);
	date_row(_("Targeted Start Date for the Execution of Work Order") . ":", 'ExeDate', '', null);
	date_row(_("Targeted close Date of the Work Order") . ":", 'RequDate', '', null, $SysPrefs->default_wo_required_by());
	qty_row(_("Targeted Production per Day in MT:"), 'qty_per_day', null, null, null, $dec);
}
else
{
...
textarea_row(_("Any Specific Information for the Work Order:"), 'memo_', null, 40, 5);
