...
***********************************************************************/
$page_security = 'SA_MANUF_EXTRUDER_COILING';
$path_to_root = "..";
...
page(_($help_context = "Extruder Coiling Entry"), false, false, "", $js);
...
{
	if (!can_process())
		unset($_POST['ProcessExtruderCoiling']);
	$Ajax->activate('_page_body');
}

if (isset($_POST['CancelExtruderCoiling']))
{
	//$selected_id = -1;
	$Ajax->activate('_page_body');
}
...
function can_process()
{
	...
	if (strlen($_POST['coiler_no']) == 0) 
	{
		display_error( _("The Coiler No  must be entered."));
		set_focus('coiler_no');
		return false;
	}
	
	if (strlen($_POST['extrudercoiling_coil_no']) == 0) 
	{
		display_error( _("The Final Coil No  must be entered."));
		set_focus('extrudercoiling_coil_no');
		return false;
	}
	...
	if(extrudercoiling_coil_no_exist($_POST['extrudercoiling_coil_no']))
	{
...
if ($selected_id != '')
{ 
	if (isset($_POST['ConfirmExtruderCoiling']) && can_process())
{
			add_extrudercoiling_details($selected_id,$_POST['date_'],$_POST['shift_id'],$_POST['coiler_no'],$_POST['operator_id'],$_POST['supervisor_id'],$_POST['rm_coil_no'],$_POST['rm_take_up_no'],$_POST['rm_length'],$_POST['rm_size_id'],$_POST['rm_qty'],$_POST['in_time'],$_POST['out_time'],trim($_POST['extrudercoiling_coil_no']),$_POST['size_id'],input_num('length_in_mtr'),input_num('qty'),input_num('steel_qty'),input_num('grease_qty'),input_num('hdpe_qty'),$_POST['remarks']);
 ...
 start_form();
   ...
   display_extrudercoiling_details($selected_id);
	//--------------------------------------------------------------------------------------
	echo '<br>';
...
{
	$extruder_coil_details=get_extruder_coil_details($_POST['rm_coil_no'],$selected_id);
	

	$take_up_no=$extruder_coil_details['take_up_no'];
	$weight=$extruder_coil_details['qty'];
	$size=$extruder_coil_details['size'];
	$size_id=$extruder_coil_details['size_id'];
	$remaining_length=$extruder_coil_details['length_in_mtr']-$extruder_coil_details['used_length'];
	$Ajax->activate('_page_body');
}
	...
	text_row(_("Coiler No.:"), "coiler_no",null,20,50);
	...
	extruder_coil_list_row(_("Coil No:"), 'rm_coil_no', null, true,true,$selected_id);
	label_row(_("Take Up No:"),$take_up_no);
	hidden('rm_take_up_no',$take_up_no);
	...
	set_no_text_row(_("Final Coil No.:"), "extrudercoiling_coil_no",null, null,null,true);
	...
	end_outer_table(1);
	if (!isset($_POST['ProcessExtruderCoiling']))
	submit_center('ProcessExtruderCoiling', _("Add Extruder Coiling Entry"), true, '', 'default');
    else
	{
	display_warning(_("Are you sure you want to save this transaction ? This action cannot be undone."), 0, 1);
   			br();
    		submit_center_first('ConfirmExtruderCoiling', _("Proceed"), '', true);
    		submit_center_last('CancelExtruderCoiling', _("Cancel"), '', 'cancel');
	}
	end_form();
}
end_page();
