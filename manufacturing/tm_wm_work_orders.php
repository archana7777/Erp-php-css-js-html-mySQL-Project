...
function edit_link($row)
{
	return  $row['closed'] ? '<i>'._('Closed').'</i>' :
		$row['wm_tm_status']?'<i>'._('Works Manager Updated').'</i>':trans_editor_link(ST_WORKORDER, $row["id"]);
}
...
{
	if($row["approved"])
	return 	'<i>'._('Approved').'</i>';
	else
	return !$row["closed"] && $row["wm_tm_status"] && !$row["approved"]? pager_link(_('Approve'),
			"/manufacturing/work_order_approve.php?trans_no=" .$row["id"]) :'<i>'._('').'</i>';
}
...
$sql = get_sql_for_tm_wm_work_orders($outstanding_only, get_post('SelectedStockItem'), get_post('StockLocation'),
...
$cols = array(
	...
	_("Per Day") => array('fun'=>'dec_amount', 'align'=>'right'),
	_("Manufactured") => array('fun'=>'dec_amount', 'align'=>'right'),
	_("Date") => array('name'=>'date_', 'type'=>'date', 'ord'=>'desc'),
    _("Execute By") => array('type'=>'date', 'ord'=>'desc'), 	
	_("Required By") => array('type'=>'date', 'ord'=>''),
	_("WM Status") =>array('insert'=>true, 'fun'=> 'edit_link'),
