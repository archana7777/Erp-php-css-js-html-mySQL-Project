...
function copy_to_cart()
{
	...
	$cart->shipping_bill_no = $_POST['shipping_bill_no'];
	...
	$cart->offer_validity_text = $_POST['offer_validity_text'];
    $cart->quote_subject=$_POST['quote_subject'];
    $cart->product_type=$_POST['product_type'];
	$cart->kind_atten=$_POST['kind_atten'];
	 
	
	if( $cart->trans_type==ST_SALESQUOTE)
	{
		$cart->technical_std_text = $_POST['technical_std_text'];
		$cart->fright_charges_text = $_POST['fright_charges_text'];
	    $cart->gst_text = $_POST['gst_text'];
	    $cart->for_text = $_POST['for_text'];
	    $cart->inspection_text = $_POST['inspection_text'];
		$cart->payment_text = $_POST['payment_text'];
		$cart->Dispatch_text = $_POST['Dispatch_text'];
...
function copy_from_cart()
{
	...
	$_POST['offer_validity_text']=$cart->offer_validity_text;
	$_POST['quote_subject']=$cart->quote_subject;
	$_POST['product_type']=$cart->product_type;
	$_POST['kind_atten']=$cart->kind_atten;
	
	if( $cart->trans_type==ST_SALESQUOTE)
	{
		$_POST['technical_std_text']=$cart->technical_std_text;
	
		$_POST['fright_charges_text']=$cart->fright_charges_text;
	    $_POST['gst_text']=$cart->gst_text ;
	    $_POST['for_text']=$cart->for_text ;
	    $_POST['inspection_text']=$cart->inspection_text ;
		$_POST['payment_text']=$cart->payment_text;
		$_POST['Dispatch_text']=$cart->Dispatch_text;
...
function can_process() {
	...
	{
		if (!get_post('quote_subject')) 
	{
		display_error(_("please enter the subject."));
		set_focus('quote_subject');
		return false;
	} 
		if (!get_post('product_type')) 
	{
		display_error(_("please enter the product type."));
		set_focus('product_type');
		return false;
	} 
	    	if (!get_post('technical_std_text')) 
	{
		display_error(_("please enter the technical standard."));
		set_focus('technical_std_text');
		return false;
	} 
	
		if (!get_post('fright_charges_text')) 
	{
		display_error(_("please enter the fright charges."));
		set_focus('fright_charges_text');
		return false;
	} 
		if (!get_post('gst_text')) 
	{
		display_error(_("please enter the gst."));
		set_focus('gst_text');
		return false;
	} 
			if (!get_post('for_text')) 
	{
		display_error(_("please enter the F.O.R."));
		set_focus('for_text');
		return false;
	} 
			if (!get_post('inspection_text')) 
	{
		display_error(_("please enter the inspection text."));
		set_focus('inspection_text');
		return false;
	} 
			if (!get_post('payment_text')) 
	{
		display_error(_("please enter the payment."));
		set_focus('payment_text');
		return false;
	} 
				if (!get_post('Dispatch_text')) 
	{
		display_error(_("please enter dispatch."));
		set_focus('Dispatch_text');
		return false;
	} 
				if (!get_post('offer_validity_text')) 
	{
		display_error(_("please enter offer validity time period."));
		set_focus('offer_validity_text');
		return false;
	} 
	}
	if($_SESSION['Items']->trans_type!= ST_SALESQUOTE)
		...
		if (!get_post('terms_of_delevery')) 
	{
	 ...
	 if($_SESSION['Items']->trans_type == ST_CUSTDELIVERY  || $_SESSION['Items']->trans_type == ST_SALESINVOICE)
	 {
	...
	{
		display_error(_("please enter the delivery note no."));
	...
	{
		display_error(_("please enter the driver name."));
	...
	{
		display_error(_("please enter the driver number."));
...
function check_item_data()
{
	...
	$coil_no_req=get_item_coil_number_required(get_post('stock_id'));
	
	if (strlen(($coil_no_req==1) && !check_num('number_of_packages', 1)))
	{
		display_error(_("The number of packages can not be empty."));
		set_focus('number_of_packages');
		return false;
	}
    ...
    $flag=0;
	foreach ($_SESSION['Items']->line_items as $item)
	{
		if($item->number_of_packages==get_post('number_of_packages'))
		$flag=1;	
    }
	/*
	if($flag)
	{
		display_error("Number of packages already in cart");
	   set_focus('number_of_packages');
	   return false;
	}
	*/
...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Items']->update_cart_item($_POST['LineNo'],
		 ...
		 input_num('Disc') / 100, $_POST['item_description'],input_num('number_of_packages'),$_POST['due_on']);
	}
	page_modified();
  line_start_focus();
}
...
function handle_new_item()
{
	...
	add_to_order($_SESSION['Items'], get_post('stock_id'), input_num('qty'),
		input_num('price'), input_num('Disc') / 100, get_post('stock_id_text'),input_num('number_of_packages'),$_POST['due_on']);

	unset($_POST['_stock_id_edit'], $_POST['stock_id'],$_POST['number_of_packages'],$_POST['due_on']);
	page_modified();
	line_start_focus();
}
