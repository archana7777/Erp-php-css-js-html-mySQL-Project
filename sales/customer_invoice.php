...
if (list_updated('tds_applicable_status'))
{
...
function check_quantities()
{
	...
	foreach ($_SESSION['Items']->line_items as $line_no=>$itm) {
		...
		if (isset($_POST['number_of_packages'.$line_no])) {
				$_SESSION['Items']->line_items[$line_no]->number_of_packages =
				  input_num('number_of_packages'.$line_no);
...
function check_no_of_packages(){
	
	$ok =1;
	// Update cart delivery quantities/descriptions
	foreach ($_SESSION['Items']->line_items as $line=>$itm) {
		//if (isset($_POST['number_of_packages'.$line])) {
		
		$coil_no_req=get_item_coil_number_required($itm->stock_id);
	
	if (strlen(($coil_no_req==1) && !(check_num('number_of_packages'.$line, 1))))
	{
		
		display_error(_("The number of packages can not be empty."));
		set_focus('number_of_packages'.$line);
		return false;
	}
...
function copy_to_cart()
{
	...
	$cart->port_code=$_POST['port_code'];
	$cart->shipping_bll_no=$_POST['shipping_bll_no'];
	$cart->shipping_bill_date=$_POST['shipping_bill_date'];
	
	$cart->tds_applicable_status=$_POST['tds_applicable_status'];
	$cart->tds_acc=$_POST['tds_acc'];
	$cart->tds_amount=$_POST['tds_amount'];
	
	$cart->dispatch_through=$_POST['dispatch_through'];
	$cart->destination=$_POST['destination'];
	$cart->terms_of_delevery=$_POST['terms_of_delevery'];
	$cart->buyers_order_date=$_POST['buyers_order_date'];
	$cart->despatch_document_no=$_POST['despatch_document_no'];
	$cart->bill_of_landing_lr_rr_no=$_POST['bill_of_landing_lr_rr_no'];
    $cart->motor_vehicle_no=$_POST['motor_vehicle_no'];
	
	$cart->delivery_note_no=$_POST['delivery_note_no'];
	$cart->driver_name=$_POST['driver_name'];
	$cart->driver_number=$_POST['driver_number'];
...
function copy_from_cart()
{
	...
	$_POST['port_code'] = $cart->port_code;
	$_POST['shipping_bill_no'] = $cart->shipping_bill_no;
	$_POST['shipping_bill_date'] = $cart->shipping_bill_date;
	
	$_POST['tds_applicable_status'] = $cart->tds_applicable_status;
	$_POST['tds_acc'] = $cart->tds_acc;
	$_POST['tds_amount'] = $cart->tds_amount;
	
	$_POST['dispatch_through'] = $cart->dispatch_through;
	$_POST['destination'] = $cart->destination;
	$_POST['terms_of_delevery'] = $cart->terms_of_delevery;
	$_POST['buyers_order_date'] = $cart->buyers_order_date;
	$_POST['despatch_document_no'] = $cart->despatch_document_no;
	$_POST['bill_of_landing_lr_rr_no'] = $cart->bill_of_landing_lr_rr_no;
	$_POST['motor_vehicle_no'] = $cart->motor_vehicle_no;
	
	$_POST['delivery_note_no'] = $cart->delivery_note_no;
	$_POST['driver_name'] = $cart->driver_name;
	$_POST['driver_number'] = $cart->driver_number;
	}
...
function check_data()
{
	...
	if (!get_post('dispatch_through')) 
	{
		display_error(_("Please enter dispatch through."));
		set_focus('dispatch_through');
		return false;
	} 
	
	
			if (!get_post('destination')) 
	{
		display_error(_("Please enter destination."));
		set_focus('destination');
		return false;
	} 
	
		if (!get_post('terms_of_delevery')) 
	{
		display_error(_("Please enter delivery terms."));
		set_focus('terms_of_delevery');
		return false;
	} 
	
	if (!get_post('despatch_document_no')) 
	{
		display_error(_("Please enter dispatch document no."));
		set_focus('despatch_document_no');
		return false;
	} 
	
	if (!get_post('bill_of_landing_lr_rr_no')) 
	{
		display_error(_("Please enter bill of landing lr-rr no."));
		set_focus('bill_of_landing_lr_rr_no');
		return false;
	} 
	
	if (!get_post('motor_vehicle_no')) 
	{
		display_error(_("Please enter motor vehicle no."));
		set_focus('motor_vehicle_no');
		return false;
	} 
	
	if (!get_post('delivery_note_no')) 
	{
		display_error(_("Please enter delivery note no."));
		set_focus('delivery_note_no');
		return false;
	} 
	
	if (!get_post('driver_name')) 
	{
		display_error(_("Please enter driver name."));
		set_focus('driver_name');
		return false;
	} 
	
	if (!get_post('driver_number')) 
	{
		display_error(_("Please enter delivery number."));
		set_focus('driver_number');
		return false;
	} 
...
start_row();
text_cells(_("Port Code:"), 'port_code', $_SESSION['Items']->port_code, 25, 25,
		    _('Port Code'), "class='tableheader2'");
end_row();
	start_row();			
text_cells(_("Shipping Bill No:"), 'shipping_bill_no', $_SESSION['Items']->shipping_bill_no, 25, 25,
		    _('Shipping Bill No'), "class='tableheader2'");
end_row();
	start_row();
date_cells(_("Shipping Bill Date"), 'shipping_bill_date', '', null, 0, 0, 0, "class='tableheader2'");
end_row();

start_row();
text_cells(_("Buyer Order No:"), 'cust_ref', $_SESSION['Items']->cust_ref, 25, 25,
		    _('Buyer Order No'), "class='tableheader2'");
end_row();	
		
start_row();			
date_cells(_("Buyer Order Date"), 'buyers_order_date','', null, 0, 0, 0, "class='tableheader2'");
end_row();				
start_row();			
text_cells(_("Dispatch Through:"), 'dispatch_through', $_SESSION['Items']->dispatch_through, 25, 25,
		    _('Dispatch Through'), "class='tableheader2'");
end_row();
...
start_row();
text_cells(_("Destination:"), 'destination', $_SESSION['Items']->destination, 25, 25,
		    _('Destination'), "class='tableheader2'");
end_row();	
start_row();		
text_cells(_("Terms of Delivery:"), 'terms_of_delevery', $_SESSION['Items']->terms_of_delevery, 25, 25,
		    _('Terms of Delivery'), "class='tableheader2'");
end_row();	
start_row();
text_cells(_("Despatch Document No.:"), 'despatch_document_no', $_SESSION['Items']->despatch_document_no, 25, 25,
		    _('Despatch Document No'), "class='tableheader2'");
end_row();	
...
start_row();
text_cells(_("Bill of Landing/LR-RR No.:"), 'bill_of_landing_lr_rr_no', $_SESSION['Items']->bill_of_landing_lr_rr_no, 25, 25,
		    _('Bill of Landing/LR-RR No'), "class='tableheader2'");
end_row();	

start_row();
text_cells(_("Motor Vehicle No.:"), 'motor_vehicle_no', $_SESSION['Items']->motor_vehicle_no, 25, 25,
		    _('Terms of Delivery'), "class='tableheader2'");
end_row();	

start_row();		
text_cells(_("Delivery Note No:"), 'delivery_note_no', $_SESSION['Items']->delivery_note_no, 25, 25,
		    _('Delivery note no'), "class='tableheader2'");
end_row();

start_row();
text_cells(_("Driver's Name:"), 'driver_name', $_SESSION['Items']->driver_name, 25, 25,
		    _('Driver Name'), "class='tableheader2'");
end_row();

start_row();
text_cells(_("Driver's Number:"), 'driver_number', $_SESSION['Items']->driver_number, 25, 25,
		    _('Driver Number'), "class='tableheader2'");
end_row();
end_outer_table();
...
if ($prepaid)
	...
	$th = array(_("Item Code"), _("Item Description"), _("Delivered"), _("Units"), _("Invoiced"),
		_("Number of Packages"),_("This Invoice"), _("Price"), _("Tax Type"), _("Discount"), _("Total"));
...
foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
	...
	$_POST['number_of_packages'.$line]=$ln_itm->number_of_packages;
   //display_error($ln_itm->number_of_packages);
    small_qty_cells(null, 'number_of_packages'.$line, qty_format($ln_itm->number_of_packages, '', $dec), null, null, $dec);
...
$taxes = $_SESSION['Items']->get_gst_taxes(input_num('ChargeFreightCost'));
