...
***********************************************************************/
$page_security = 'SA_SALESDELIVERY_PACKING';
$path_to_root = "..";
...
check_db_has_sales_pending_packing_items(_("There are no pending packing list for delivery items in the system."));
...
function display_pending_packing_list_items_roll_nos($selected_parent)
{
	...
	$result = get_packing_list_coil_nos($selected_parent);
	div_start('bom');
...
if (isset($_POST['FINAL']))
{
	...
	$grn_sqm=get_total_no_of_packages_in_grn($selected_parent); //grn quantity 
	...
	if ($packing_list_required==1)
	{
		...
		$packing_weight=get_packing_list_total_weight($selected_parent);
		
		if($grn_sqm<=$packing_weight){
		
			update_final_submit_sales_delivery_packing($selected_parent);
			display_notification(_('Selected batch code has been updated'));
			$Mode = 'RESET';
		}else
			{
				
			display_error("Packing list quantity  and delivery quantity are not equal! Delivery quantity is ".$grn_sqm." and packing quantity is ".$packing_weight);
			return false;
		}
	}
	/*
...
function on_submit($selected_parent, $selected_roll_number=-1)
{
	...
	elseif (strlen($_POST['coil_number']) == 0) 
	{
	 ...
	 $stock_coil_pass_no_check=check_exist_delivery_packing_list_Coil_no_in_stock($_POST['coil_number']);
   if($stock_coil_pass_no_check ==0)
   {
	...
	$packing_coil_no_check=check_exist_sales_delivery_packing_list_coil_no($_POST['coil_number']);
   if($packing_coil_no_check >0)
   {
	...
	$grn_sqm=get_total_no_of_packages_in_grn($selected_parent);  //grn quantity  either sqm or weight
	$grn_det=get_packing_list_item_code($selected_parent);
	...
	if ($packing_list_required==1)
	{
		...
		{
		display_error(_("The total no of packages is more than the delivered no of packages And the delivery no of packages is ".$grn_sqm));
		set_focus('no_of_packages');
		return;
		}
	...
	*/
	if ($selected_roll_number != -1)
	{
		...
		update_packing_list_coil_number($selected_roll_number,$_POST['coil_number'], $_POST['no_of_packages'],$_POST['remarks'],$process_date);
	...
	else
	{
		...
		add_packing_list_coil_number($selected_parent, $_POST['coil_number'], $_POST['no_of_packages'],$_POST['remarks'],$process_date);
...
if ($Mode == 'Delete')
{
	delete_packing_list_coil_no($selected_id);
...
sales_pending_packing_list_items_cells(_("Select a Packing List item:"), 'stock_id', null, false, true);
...
start_form();
	...
	if ($selected_id != -1)
	{
 		if ($Mode == 'Edit') {
			...
			$myrow = get_packing_list_item_coil_no($selected_id);
	...
	if(!empty($_POST['coil_number'])){
		...
		$roll_info=get_coil_number_info_from_stock_moves($_POST["coil_number"]);
