...
function prtopt_link($row)
{

  	if ($row['type'] == ST_SALESINVOICE){ 
		salesinv_options_cells(_(""),'inv_options'.$row['trans_no'],$_POST['inv_options'],true);
		$selected_option=$_POST['inv_options'.$row['trans_no']];
		
		return print_document_link($row['trans_no']."-".$row['type'], _("Print"), true, $row['type'], ICON_PRINT,'','',0,0,$selected_option);	
	}
	else{
		return '';
	}
