...
function proforma_invoice_link($row)
{
	global $trans_type;
	if ($trans_type == ST_SALESORDER)
  		return pager_link( _("Proforma Invoice"),
			"/sales/sales_proforma_invoice.php?OrderNumber=" .$row["order_no"], ICON_DOC);
	else
		return '';
}	
	...
	array_append($cols, array(
	 ...
	 array_append($cols,array(
			_("Tmpl") => array('insert'=>true, 'fun'=>'tmpl_checkbox'),
					array('insert'=>true, 'fun'=>'edit_link'),
					array('insert'=>true, 'fun'=>'prt_link'),
					array('insert'=>true, 'fun'=>'proforma_invoice_link')));
};
