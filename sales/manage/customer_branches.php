...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	// ravi
	if(($_POST['business_type']==1) && ($_POST['branch_gst']==''))
	{
		display_error(_("Please Enter the GST Number."));
		$input_error = 1;
	}
	
	if((($_POST['business_type']==1)||($_POST['business_type']==2)) && ($_POST['state_code']==-1))
	{
		display_error(_("Please Select State Code."));
		$input_error = 1;
	}
	
	if((($_POST['tax_payer_type']==5)||($_POST['tax_payer_type']==6)) && ($_POST['business_type']==1))
	{
		display_error(_("Incorrect Selection of Business Type and Tax Payer Type."));
		$input_error = 1;
	}
	
	if(($_POST['tax_payer_type']==5) && (($_POST['business_type']==1)|| ($_POST['business_type']==2)))
	{
		display_error(_("Incorrect Selection of Business Type and Tax Payer Type."));
		$input_error = 1;
	}
	
	if(($_POST['tax_payer_type']!=5) &&($_POST['business_type']==3)|| ($_POST['business_type']==4))
	{
		display_error(_("Incorrect Selection of Business Type and Tax Payer Type."));
		$input_error = 1;
	}
	...
	{
		display_error(_("Cannot Edit this branch because customer transactions have been created to this branch."));

	}
	if (branch_in_foreign_table($_POST['customer_id'], $_POST['branch_code'], 'sales_orders'))
		{
			display_error(_("Cannot edit this branch because sales orders exist for it. Purge old sales orders first."));
		}
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1)
		{
			update_branch($_POST['customer_id'], $_POST['branch_code'], $_POST['br_name'], $_POST['br_ref'],
				...
				$_POST['default_ship_via'], $_POST['notes'], $_POST['bank_account'], $_POST['branch_gst'],
				$_POST['business_type'], $_POST['tax_payer_type'], $_POST['state_code'],$_POST['pin_code'],$_POST['tds_applicable'],$_POST['tds_service_type'],$_POST['pan_no'],$_POST['pan_status'],$_POST['name_on_pan']);
		...
		else
		{
			add_branch($_POST['customer_id'], $_POST['br_name'], $_POST['br_ref'],
				...
				$_POST['sales_discount_account'], $_POST['receivables_account'], $_POST['payment_discount_account'],
				$_POST['default_location'], $_POST['br_post_address'], $_POST['group_no'],
				$_POST['default_ship_via'], $_POST['notes'], $_POST['bank_account'],$_POST['branch_gst'],$_POST['business_type'],$_POST['tax_payer_type'],$_POST['state_code'],$_POST['pin_code'],$_POST['tds_applicable'],$_POST['tds_service_type'],$_POST['pan_no'],$_POST['pan_status'],$_POST['name_on_pan']);
...
function branch_settings($selected_id, $num_branches) {
	...
	if ($selected_id != -1)
	{
	 	if ($Mode == 'Edit' || !isset($_POST['br_name'])) {
			...
			$_POST['branch_gst'] = $myrow['branch_gst'];
			$_POST['business_type']  = $myrow["business_type"];
			$_POST['tax_payer_type']  = $myrow["tax_payer_type"];
			$_POST['state_code']  = $myrow["state_code"];
			$_POST['pin_code'] = $myrow['pin_code'];
			$_POST['tds_applicable']  = $myrow["tds_applicable"];
			$_POST['pan_no']  = $myrow["pan_no"];
			$_POST['pan_status']  = $myrow["pan_status"];
			$_POST['name_on_pan']  = $myrow["name_on_pan"];
			$_POST['tds_service_type']  = $myrow["tds_service_type"];
	...
	table_section_title(_("Sales and Taxation"));
	...
	text_row(_("GST No:"), 'branch_gst', null, 20, 20);
	text_row(_("Pin Code:"), 'pin_code', null, 20, 20);
	business_types_list_row(_("Business Type :"), 'business_type', null,true);
	tax_payer_types_list_row(_("Tax Payer Type :"), 'tax_payer_type', null,true);
	gst_state_code_list_row(_("State Codes :"), 'state_code', null,"Select State Code");
	
	table_section_title(_("TDS Details"));
		yesno_list_row(_("TDS Applicable:"), 'tds_applicable', null, "", "", true);
		if($_POST['tds_applicable']==1)
		{
			tds_service_types_list_row(_("Customer Service Type:"), 'tds_service_type', null,true);
			text_row(_("PAN / IT No:"), 'pan_no', null, 32, 30);
			pan_status_types_list_row(_("PAN Status:"), 'pan_status', null,true);
			text_row(_("Name On PAN Card:"), 'name_on_pan', null, 32, 30);
		}
