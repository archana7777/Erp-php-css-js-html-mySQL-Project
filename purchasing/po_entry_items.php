...
function check_data()
{
	...
	if ($_SESSION['PO']->trans_type == ST_SUPPINVOICE || $_SESSION['PO']->trans_type == ST_SUPPRECEIVE){ 
		
	$coil_no_req=get_item_coil_number_required($_POST['stock_id']);
	
	if (strlen(($coil_no_req==1) && !check_num('no_of_packages', 1)))
	{
...
function handle_update_item()
{
	...
	if ($allow_update)
	{
		...
		$_SESSION['PO']->update_order_item($_POST['line_no'], input_num('qty'), input_num('price'),
  			@$_POST['req_del_date'], $_POST['item_description'],input_num('no_of_packages'));
		unset_form_variables();
	}	
    line_start_focus();
}
		...
		if ($allow_update == true)
		{
			...
			if ($allow_update)
			{
				$_SESSION['PO']->add_to_order (count($_SESSION['PO']->line_items), $_POST['stock_id'], input_num('qty'), 
					...
					$_SESSION['PO']->trans_type == ST_PURCHORDER ? $_POST['req_del_date'] : '', 0, 0,input_num('no_of_packages'));
...
function can_commit()
{
	...
	{
		if(!get_post('receipt_doc_lr')) {
		display_error( _("Receipt l/R  cannot be empty."));
		set_focus('stock_id_edit');
		return false;
		}
		if(!get_post('destination')) {
		display_error( _("Destination  cannot be empty."));
		set_focus('destination');
		return false;
		}
		if(!get_post('terms_of_delivery')) {
		display_error( _("Please enter the Terms of Delivery."));
		set_focus('terms_of_delivery');
		return false;
		}
		if(!get_post('dispatch_through')) {
		display_error( _("Please enter the Dispatch Through."));
		set_focus('dispatch_through');
		return false;
		}
	}
		if ($_SESSION['PO']->trans_type == ST_PURCHORDER)
		{
			if(!get_post('packing_charges_text')) {
		       display_error( _("Please enter the Packing Charges."));
		       set_focus('packing_charges_text');
		       return false;
			   }
			 ...
			 if(!get_post('gst_text')) {
		       display_error( _("Please enter the  G.S.T."));
		       set_focus('gst_text');
		       return false;
			   }  
			 if(!get_post('fright_charges_text')) {
              ...
              if(!get_post('payment_text')) {
               ...
               if(!get_post('delivery_text')) {
		       display_error( _("Please enter the Delivery."));
		       set_focus('delivery_text');
		       return false;
			   } 
               if(!get_post('transporter_text')) {
		       display_error( _("Please enter the Transporter Details."));
		       set_focus('transporter_text');
		       return false;
			   }     			   
