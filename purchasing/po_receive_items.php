...
function display_po_receive_items()
{
    ...
    $th = array(_("Item Code"), _("Description"), _("Ordered"), _("Units"), _("Received"),
    	_("Outstanding"), _("No of Packages"),_("This Delivery"), _("Price"), _("Total"));
    table_header($th);
    ...
    if (count($_SESSION['PO']->line_items)> 0 )
    {
          ...
          small_qty_cells(null, 'no_of_packages'.$ln_itm->line_no, qty_format($ln_itm->no_of_packages, '', $dec), null, null, $dec);
	...
	$taxes = $_SESSION['PO']->get_gst_taxes(input_num('freight_cost'), true);
...
function can_process()
{
	...
	/*
	$coil_no_req=get_item_coil_number_required($order_line->stock_id);
	if (strlen(($coil_no_req==1) && !check_num($order_line->no_of_packages, 1)))
	{
		display_error(_("The no of packages can not be empty."));
...
if (isset($_POST['Update']) || isset($_POST['ProcessGoodsReceived']))
{
	...
	foreach ($_SESSION['PO']->line_items as $line)
	{
	 if( ($line->quantity - $line->qty_received)>0) {
		...
		$_SESSION['PO']->line_items[$line->line_no]->no_of_packages = input_num('no_of_packages'.$line->line_no);
