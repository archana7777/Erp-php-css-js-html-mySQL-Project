...
function print_inventory_purchase()
{
	...
	$headers = array(_('S.No'), _('Grade'), _('Description'),('Inactive'));
	if ($fromsupp != '')
		$headers[4] = '';
    ...
    $rep = new FrontReport(_('Grade Listing '), "GradeListingReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($trans=db_fetch($res))
	{
			...
			$rep->TextCol(0, 1, $i);
			$rep->TextCol(1, 2, $trans['grade']);
			$rep->TextCol(2, 3, $trans['description'] );
			$rep->TextCol(3, 4, $trans['inactive'] );
