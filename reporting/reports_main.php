...
$reports->addReport(RC_CUSTOMER, 1010, _('&TDS Receivable Report'),
...
$reports->addReport(RC_CUSTOMER, 109, _('&Print Sales Orders'),
	array(	_('From') => 'ORDERS',
			...
			_('Comments') => 'TEXTBOX',
			_('Orientation') => 'ORIENTATION'));
$reports->addReport(RC_CUSTOMER, 116, _('Sales Orders'),
	array(	_('From') => 'ORDERS',
			_('To') => 'ORDERS',
			_('Currency Filter') => 'CURRENCY',
			_('Email Customers') => 'YES_NO',
...
$reports->addReport(RC_SUPPLIER, 203, _('&Payment Report'),
	array(	_('End Date') => 'DATE',
			...
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
$reports->addReport(RC_SUPPLIER, 2030, _('&TDS Payable Report'),
	array(	_('End Date') => 'DATE',
			_('Supplier') => 'SUPPLIERS_NO_FILTER',
			_('Suppress Zeros') => 'YES_NO',
			_('Comments') => 'TEXTBOX',
...
$reports->addReport(RC_INVENTORY, 310, _('Inventory Purchasing - &Transaction Based'),
	array(	_('Start Date') => 'DATEBEGINM',
			...
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
$reports->addReport(RC_INVENTORY, 311, _('Grade Listing'),
	array(	
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
$reports->addReport(RC_INVENTORY, 312, _('Size Listing'),
	array(	
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
$reports->addReport(RC_INVENTORY, 313, _('Items Listing'),
	array(	
			...
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));			
$reports->addReport(RC_INVENTORY, 314, _('Spool Listing'),
	array(	
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));			
...
$reports->addReportClass(_('Maintenance '), RC_MAINTAINACE);
if ($_SESSION["wa_current_user"]->can_access_page(SA_MACHINMAIN_REPORTS))
{
	
	$reports->addReport(RC_MAINTAINACE, 825, _('Machine Master Report'),
	array(	
			_('Machine Name') =>  'MACHINE_NAME_NO_FILTER',
			_('Model No') =>  'TEXT',
			_('Machine Equipments') =>  'MACHINE_EQUIPTMENTS_NO_FILTER',
			_('Machine Make') =>  'MACHINE_MAKE_NO_FILTER',
			_('Machine Capacity') =>  'MACHINE_CAPACITY_NO_FILTER',
			_('Supplier') => 'SUPPLIERS_NO_FILTER',
			_('Location ') => 'INVENTORY_LOCATIONS',
			_('Comments') => 'TEXTBOX',
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
...
if ($_SESSION["wa_current_user"]->can_access_page(SA_MACHINMAIN_REPORTS))
{	
	$reports->addReport(RC_MAINTAINACE, 820, _('Machine Make Report'),
	array(	
			_('Machine Make') =>  'MACHINE_MAKE_NO_FILTER',
			_('Comments') => 'TEXTBOX',
			_('Orientation') => 'ORIENTATION',
...
if ($_SESSION["wa_current_user"]->can_access_page(SA_MACHINMAIN_REPORTS))
{	
	$reports->addReport(RC_MAINTAINACE, 821, _('Machine Equipments Report'),
	array(	
			_('Machine Equipments') =>  'MACHINE_EQUIPTMENTS_NO_FILTER',
			_('Comments') => 'TEXTBOX',
			_('Orientation') => 'ORIENTATION',
...
if ($_SESSION["wa_current_user"]->can_access_page(SA_MACHINMAIN_REPORTS))
{				
	$reports->addReport(RC_MAINTAINACE, 823, _('Machine Frequency Report'),
	array(	
			_('Machine Frequency') =>  'MACHINE_FREQUENCY_NO_FILTER',
			_('Comments') => 'TEXTBOX',
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
}
if ($_SESSION["wa_current_user"]->can_access_page(SA_MACHINMAIN_REPORTS))
{			
	$reports->addReport(RC_MAINTAINACE, 824, _('Machine Maintenance Checklist Report'),
	array(	
			_('Machine Name') =>  'MACHINE_NAME_NO_FILTER',
			_('Machine Frequency') =>  'MACHINE_FREQUENCY_NO_FILTER',
			_('Comments') => 'TEXTBOX',
...
if ($_SESSION["wa_current_user"]->can_access_page(SA_MACHINMAIN_REPORTS))
{			
	$reports->addReport(RC_MAINTAINACE, 804, _('Preventive Maintenance Reports'),
	array(	_('Start Date') => 'DATEBEGINM',
			_('End Date') => 'DATEENDM',
			_('Machine Name') => 'MACHINENAMESTOCK',
			_('Comments') => 'TEXTBOX',
			_('Orientation') => 'ORIENTATION',
			_('Destination') => 'DESTINATION'));
}
