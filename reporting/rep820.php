...
{
	$sql = "SELECT * FROM ".TB_PREF."machine_make WHERE 1=1 ";
     if ($testtype == -1)
 	 $sql .= " AND !inactive";
		else
...
function print_outstanding_GRN()
{
	...
	$cols = array(0, 40,200,400);

	$headers = array(_('S.No'), _('Machine Make'),   _('Description'));
    ...
    $rep = new FrontReport(_('Machine Make Report'), "machinemakereport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	While ($testtype = db_fetch($res))
	{
		...
		$rep->NewLine();
		$rep->TextCol(0, 1, $k);
		$rep->TextCol(1, 2, $testtype['mac_make']);
