...
function getTransactions($machine,$machine_eqp,$machine_make,$machine_capacity,$supplier_id,$loc_grp,$location,$model_no="")
{
	
 $sql = "SELECT machine.*,
   location.location_name as location,
   loc_group.loc_group_name as group_des,
...
LEFT JOIN
    ".TB_PREF."location_groups loc_group ON loc_group.id=machine.loc_group_id
LEFT JOIN
...
LEFT JOIN
 ...
 LEFT JOIN 
  ...
  if ($loc_grp != '')
   $sql .= " AND machine.loc_group_id =".db_escape($loc_grp);
...
function get_location_group($loc_grp)
{
		$sql="SELECT loc_group_name FROM ".TB_PREF."location_groups WHERE id = '$loc_grp' ";
...
function print_outstanding_GRN()
{
	...
	$loc_grp=get_location_group($loc_group);
    ...
    $params =   array( 	0 => $comments,
    				    1 => array('text' => _('Location Group'), 'from' => $loc_grp, 'to' => ''),
