...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error !=1) {
    	write_item_unit($selected_id, $_POST['abbr'], $_POST['description'], $_POST['decimals'],$_POST['quantity_type']);
		if($selected_id != '')
...
$th = array(_('UQC Code'), _('Description'),_('Quantity Type'), _('Decimals'), "", "");
inactive_control_column($th);

table_header($th);
$k = 0; //row colour counter

global $quantity_types;
...
while ($myrow = db_fetch($result))
{
	...
	label_cell($quantity_types[$myrow["quantity_type"]]);
