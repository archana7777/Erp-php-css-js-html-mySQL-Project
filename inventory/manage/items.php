...
check_db_has_size(_("Item Size is not defined!. Please enter at least one Size in Size master."));
check_db_has_grade(_("Item Grade is not defined!. Please enter at least one Grade in Grade master"));
function clear_data()
{
	...
	unset($_POST['size_id']);
	unset($_POST['inward_qc_check_req']);
	unset($_POST['coil_no_req']);
	unset($_POST['inward_qc_template_id']);
	...
	unset($_POST['tax_slab_for_good']);
	unset($_POST['tax_slab_for_service']);
	unset($_POST['cess_percentage']);
	unset($_POST['hsn_code']);
	unset($_POST['sac_code']);
	unset($_POST['grade_id']);
	unset($_POST['process_id']);
	unset($_POST['product_type_id']);
	unset($_POST['standard']);
	unset($_POST['product_class']);
	unset($_POST['oil_status']);
}
	...
	if ($input_error != 1)
	{
		...
		{ /*so its an existing one */
			update_item($_POST['NewStockID'], $_POST['description'],
				...
				get_post('fa_class_id'),input_num('tax_slab_for_good'),input_num('tax_slab_for_service'),input_num('cess_percentage'),$_POST['hsn_code'],$_POST['sac_code'],check_value('coil_no_req'),check_value('inward_qc_check_req'),$_POST['inward_qc_template_id'],$_POST['size_id'],$_POST['grade_id'],$_POST['process_id'],$_POST['product_type_id'],$_POST['standard'],$_POST['product_class'],$_POST['oil_status']);
		...
		{ //it is a NEW part
			...
			$_POST['no_sale'] = $_POST['editable'] = $_POST['no_purchase'] = $_POST['no_purchase'] = 0;
			set_focus('NewStockID');
		}
		$Ajax->activate('_page_body');
	}
...
function item_settings(&$stock_id, $new_item) 
{
	...
	if ($new_item) 
	{
		...
		text_row(_("Item / Part Code:"), 'NewStockID', $tmpCodeID, 21, 20, null, "", $post_label);
		$_POST['inactive'] = 0;
	} 
	else 
	{ // Must be modifying an existing item
		...
		label_row(_("Item/Part Code:"),$_POST['NewStockID']);
		hidden('NewStockID', $_POST['NewStockID']);
		set_focus('description');
	}
	$fixed_asset = get_post('fixed_asset');

	text_row(_("Item / Part Name:"), 'description', null, 52, 200);
	...
	if(!get_post('fixed_asset'))
		...
		get_size_list_row(_("Size:"),'size_id');
	
	if(get_post('category_id')==11)
	{
		production_processes_list_row(_("Production Process:"), 'process_id', null, true);
	}
	if(get_post('category_id')==2)
	{
		product_types_list_row(_("Product Type:"), 'product_type_id');
		stock_grade_list_row(_("Grade:"),'grade_id');
		text_row(_("Indian Standard:"), 'standard', null, 15, 20);
		text_row(_("Class:"), 'product_class', null, 15, 20);
		oil_status_types_list_row("Oil Status",'oil_status');
	}
	if(get_post('category_id')==6)// done by venkat-->getting grade for raw material
	{
	  stock_grade_list_row(_("Grade:"),'grade_id');
  ...
  item_tax_types_list_row(_("Item Tax Type:"), 'tax_type_id', null, !$new_item && item_type_inactive(get_post('tax_type_id')));
	...
	if (!get_post('fixed_asset'))
		check_row(_("Coil Number Required:"), 'coil_no_req');
	 
if(get_post('category_id')==6)
{	
	if (!get_post('fixed_asset'))
	check_row(_("Inward QC Check Required:"), 'inward_qc_check_req',null,true);
  if(get_post('inward_qc_check_req'))
  {
	 if (!get_post('fixed_asset'))
		incoming_qc_templates_row(_("Inward QC Template:"), 'inward_qc_template_id',null,true);
  }	
} 
	...
	//ravi
	table_section_title(_("Tax Slab"));
	text_row(_("HSN Code:"), 'hsn_code', null, 10, 20);
	qty_row(_("Tax Slab (For Good)"),'tax_slab_for_good',0,null,"%",0);
	text_row(_("SAC Code:"), 'sac_code', null, 10, 20);
	qty_row(_("Tax Slab (For Service)"),'tax_slab_for_service',0,null,"%",0);
	qty_row(_("Cess (Percentage)"),'cess_percentage',0,null,"%",0);
