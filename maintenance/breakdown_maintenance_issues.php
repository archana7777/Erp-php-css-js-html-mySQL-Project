...
***********************************************************************/
$page_security = 'SA_MAINTENANCE_BREAKTRANS';
$path_to_root = "..";
...
include_once($path_to_root . "/maintenance/includes/item_breakdown_maintenance_ui.inc");
...
if (isset($_GET['id'])) {
	if (isset($_GET['FixedAsset'])) {
		...
		$_SESSION['page_title'] = _($help_context = "Issue Items For Breakdown Maintenance");
	}
}
...
if (isset($_GET['AddedID'])) 
{
	...
	$trans_type = ST_BREAKMAINTISSUES;
  ...
  if (is_fixed_asset($row['mb_flag'])) {
    ...
    display_notification_centered(_("Items Breakdown Maintenance has been processed"));
	
    display_note(get_trans_view_str($trans_type, $trans_no, _("&View this Breakdown Maintenance")));

    display_note(get_gl_view_str($trans_type, $trans_no, _("View the GL &Postings for this Breakdown Maintenance")), 1, 0);

	hyperlink_params($_SERVER['PHP_SELF'], _("Enter &Breakdown Maintenance"), "NewAdjustment=1");
  }
...
function handle_new_order()
{
    ...
    $_SESSION['adj_items'] = new items_cart(ST_BREAKMAINTISSUES);
...
function can_process()
{
	...
	if (!check_reference($_POST['ref'], ST_BREAKMAINTISSUES))
	{
		set_focus('ref');
		return false;
	}
...
if (isset($_POST['Process']) && can_process()){
  ...
  $fixed_asset = $_SESSION['adj_items']->fixed_asset; 
	...
	$trans_no = add_breakdown_maintenance_items($_SESSION['adj_items']->line_items,
		$_POST['StockLocation'], $_POST['AdjDate'],	$_POST['ref'], $_POST['memo_'],0, $_POST['loc_cat'],$_POST['loc_subcat'],$_POST['loc_subsubcat'],$_POST['brkd_req_id']);
...
hidden('brkd_req_id',$_GET["id"]);

display_order_header_new2($_SESSION['adj_items'],$_GET["id"]);

start_outer_table(TABLESTYLE, "width='70%'", 10);

display_maintenance_items2($items_title, $_SESSION['adj_items']);
