...
include_once($path_to_root . "/maintenance/includes/upgrade_request_ui.inc");
...
if (user_use_date_picker())
	...
	$_SESSION['page_title'] = _($help_context = "Define Items for Machine Upgrade");
...
if (isset($_GET['AddedID'])) 
{
  ...
  if (is_fixed_asset($row['mb_flag'])) {
    ...
    display_notification_centered(_("Machine Upgrade Items has been processed"));
...
function handle_new_order()
{
    ...
    $_SESSION['adj_items'] = new items_cart(ST_UPGRADEREQ);
...
function can_process()
{
	...
	if (!check_reference($_POST['ref'], ST_UPGRADEREQ))
	{
		set_focus('ref');
		return false;
	}
...
if (isset($_POST['Process']) && can_process()){
	...
	$trans_no = add_upgrder_request_items($_SESSION['adj_items']->line_items, $_POST['AdjDate'],$_POST['ref'], $_POST['memo_'],$_POST["schedule_id"],ST_UPGRADEREQ,$_POST['StockLocation'],$emp_id,$dept_id);
...
display_order_header_mac_upgrade($_SESSION['adj_items'],$_POST['id']);

start_outer_table(TABLESTYLE, "width='70%'", 10);

display_maintenance_items_upgrade($items_title, $_SESSION['adj_items']);
adjustment_options_controls_upgrade();
