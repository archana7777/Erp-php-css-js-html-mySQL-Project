...
***********************************************************************/
$page_security = 'SA_INDENTREQUEST_INQUIRY';
$path_to_root="../..";
...
if (user_use_date_picker())
	$js .= get_js_date_picker();
	 ...
	 if($trans_type == 95){
		page(_($help_context = "Indent Requests"), false, false, "", $js);
}
...
function get_all_indent_requests($from_date,$to_date,$fromDepartment)
{
	...
	WHERE req.tran_date >= '$from_date' AND req.tran_date <= '".$to_date."' and req.indent_status='0'";
	 //display_error($sql);
...
function Update_link($row)
{
	global $trans_type;
  		...
  		return pager_link( _("Indent Request Process"),
			"/sales/indent_process.php?trans_no=" .$row['trans_no'], ICON_DOC);
}
...
$sql1 = get_all_indent_requests($_POST["start_date"],$_POST["end_date"],$_POST['FromDepartment']);	
