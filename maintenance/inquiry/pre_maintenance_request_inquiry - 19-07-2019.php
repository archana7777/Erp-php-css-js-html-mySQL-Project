...
if (user_use_date_picker())
	...
	page(_($help_context = "Maintenance Request Inquiry"), false, false, "", $js);
//-----------------------------------------------------------------------------------
// Ajax updates
...
function get_preventive_maintenance_request_details($machine_code,$from,$to,$machines)
{
	...
	$sql="SELECT pmr.id,meq.mac_eqp,ma.mac_code,pmr.ref,pmr.schedule_id,pmr.req_date,pmr.process_status FROM ".TB_PREF."pre_maintenance_req as pmr JOIN ".TB_PREF."machine_maintenance_schedule as ms ON ms.id=pmr.schedule_id LEFT JOIN ".TB_PREF."machine as ma ON ma.id=ms.machine_id LEFT JOIN ".TB_PREF."machine_equipment as meq ON meq.id=ma.mac_eqp  WHERE pmr.req_date >= '$date_after' AND pmr.req_date <= '$date_before'";
