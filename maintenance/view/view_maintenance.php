...
display_heading($systypes_array[ST_MAINTISSUES] . " #$trans_no");

br(1);
$adjustment_items = get_maintenance_items($trans_no);
//display_error($adjustment_items);
$k = 0;
$header_shown = false;
while ($adjustment = db_fetch($adjustment_items))
{
	...
	if (!$header_shown)
	{
		...
		label_cells(_("Schedule Id"), $adjustment['schedule_id'], "class='tableheader2'", "colspan=6");
		
		end_row();
		comments_display_row(ST_MAINTISSUES, $trans_no);
...
is_voided_display(ST_MAINTISSUES, $trans_no, _("This adjustment has been voided."));

end_page(true, false, false, ST_MAINTISSUES, $trans_no);
