...
function get_all_checklist_details_machine_code($schedule_id)
{
	...
	/* $sql = "SELECT mck.*,if(mck.verified='','No','Yes') as verified,if(mck.remarks='','Not Defined',mck.remarks)as remarks,mmc.details FROM ".TB_PREF."mm_checklist_result as mck  JOIN ".TB_PREF."machine_maintenance_checklists as mmc ON mck.chk_list_id=mmc.id WHERE mck.sch_id=".db_escape($schedule_id)." AND mck.machine_fre_id=".db_escape($frequency_id)." AND mmc.mc_problem_type=".db_escape($mc_problem_type)."";
	$result=db_query($sql, "could not get Machine Maintenance Schedules");*/
	$sql="SELECT mms.*,mc.*,mmc.*,mcr.*,if(mcr.verified=1,'Yes','No') as verified,if(mcr.remarks='','Not Defined',mcr.remarks)as remarks FROM ".TB_PREF."machine_maintenance_schedule as mms LEFT JOIN ".TB_PREF."machine as mc ON mms.machine_id=mc.id LEFT JOIN  ".TB_PREF."machine_maintenance_checklists as mmc ON mmc.equipment_id=mc.mac_eqp LEFT JOIN ".TB_PREF."mm_checklist_result as mcr ON mcr.chk_list_id=mmc.id WHERE mms.id=".db_escape($schedule_id)." AND mms.mc_problem_type=mmc.mc_problem_type AND mms.mac_fre=mmc.machine_fre_id GROUP BY mmc.id";
	$result=db_query($sql, "could not get Machine Maintenance Schedules");
