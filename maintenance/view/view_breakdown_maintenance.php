...
display_heading($systypes_array[ST_BREAKMAINTISSUES] . " #$trans_no");

br(1);
$adjustment_items = get_breakdown_maintenance_items($trans_no);
$k = 0;
$header_shown = false;
while ($adjustment = db_fetch($adjustment_items))
{
	...
	if (!$header_shown)
	{
        $machine_name=get_machine_name_based_stock($adjustment['machine_id']);
		...
		label_cells(_("Machine Name"), $machine_name, "class='tableheader2'");
		end_row();
		comments_display_row(ST_BREAKMAINTISSUES, $trans_no);
		end_table();
		$header_shown = true;
...
is_voided_display(ST_BREAKMAINTISSUES, $trans_no, _("This Breakdown Maintenance has been voided."));

end_page(true, false, false, ST_BREAKMAINTISSUES, $trans_no);
