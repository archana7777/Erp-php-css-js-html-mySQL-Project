...
{
$sql="SELECT up_ent.*, CONCAT(meq.mac_eqp,'-',m.mac_code) as mac_eqp FROM ".TB_PREF."machine as m LEFT JOIN ".TB_PREF."machine_equipment as meq ON m.mac_eqp=meq.id left join ".TB_PREF."upgrade_req_entry up_ent on up_ent.machine_id=m.id LEFT JOIN ".TB_PREF."upgrade_material_request as req ON req.schedule_id=up_ent.id where req.id=".db_escape($trans_no)."";
...
{
	$sql="SELECT * FROM ".TB_PREF."upgrade_material_request as req INNER JOIN ".TB_PREF."upgrade_issue_entry as entry ON req.id=entry.req_id WHERE req.id=".db_escape($req_id)."";
