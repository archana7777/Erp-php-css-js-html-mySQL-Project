...
***********************************************************************/
$page_security = 'SA_MAINTENANCE_TRANSA';
$path_to_root = "..";
...
include_once($path_to_root . "/maintenance/includes/item_maintenance_ui.inc");
...
if (isset($_GET['id'])) {
	if (isset($_GET['FixedAsset'])) {
		...
		$_SESSION['page_title'] = _($help_context = "Issue Items For Preventive Maintenance");
	}
}
...
if (isset($_GET['AddedID'])) 
{
  ...
  if (is_fixed_asset($row['mb_flag'])) {
    ...
    display_notification_centered(_("Items Preventive has been processed"));
	
    display_note(get_trans_view_str($trans_type, $trans_no, _("&View this Preventive")));

    display_note(get_gl_view_str($trans_type, $trans_no, _("View the GL &Postings for this Preventive")), 1, 0);

	  hyperlink_params($_SERVER['PHP_SELF'], _("Enter &Another Preventive"), "NewAdjustment=1");
  }
...
function handle_new_order()
{
    ...
    $_SESSION['adj_items'] = new items_cart(ST_MAINTISSUES);
...
function can_process()
{
	...
	$check_schedule=getMaintenanceSchedule($_POST['schedule_id']);
	if(!$check_schedule){
		display_error(_("Schedule Id is not existing, Please Entered Valid Schedule Id"));
		set_focus('schedule_id');
		return false;
	}
	}
	if ($_POST['schedule_id']=='')	{
		display_error(_("Schedule Id should not be empty"));
		set_focus('schedule_id');
		return false;
	}
	...
	if (!check_reference($_POST['ref'], ST_MAINTISSUES))
	{
		set_focus('ref');
		return false;
	}
...
if (isset($_POST['Process']) && can_process()){
   //display_error($_POST['schedule_id']);die;
  $fixed_asset = $_SESSION['adj_items']->fixed_asset; 

	
	$trans_no = add_maintenance_items($_SESSION['adj_items']->line_items,
		$_POST['StockLocation'], $_POST['AdjDate'],	$_POST['ref'], $_POST['memo_'],0, $_POST['loc_group_id'],'','',$_POST['schedule_id']);
...
hidden('schedule_id',$_GET["id"]);

display_order_header_new1($_SESSION['adj_items'],$_GET['id']);

start_outer_table(TABLESTYLE, "width='70%'", 10);

display_maintenance_items($items_title, $_SESSION['adj_items']);
...
function getMaintenanceSchedule($schedule_id){
	$sql = "SELECT id FROM ".TB_PREF."machine_maintenance_schedule move WHERE id=".db_escape($schedule_id);
	$result= db_query($sql);
	$schedule=db_fetch_row($result);
	return $schedule[0];
}