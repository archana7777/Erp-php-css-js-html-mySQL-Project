...
include_once($path_to_root . "/maintenance/includes/db/calibration_attache_documents_db.inc");
...
if ($view_id != -1)
{
	$row = get_calibration_attachment($view_id);
	if ($row['filename'] != "")
	{
		if(in_ajax()) {
...
if ($download_id != -1)
{
	$row = get_calibration_attachment($download_id);
	if ($row['filename'] != "")
	{
...
page(_($help_context = "Attach Calibration Documents"), false, false, "", $js);
...
{   
	if (!check_transactin($_POST['trans_no'],$_POST['machine_id']))
	{
		display_error(_("Selected transaction does not exists."));
		//dislpay_error('HI');
	}
	else if($Mode == 'ADD_ITEM' && !isset($_FILES['filename'])){
	display_error(_("Select attachment file."));
		...
		if ($Mode == 'UPDATE_ITEM')
		{
		    $row = get_calibration_attachment($selected_id);
		    if ($row['filename'] == "")
        		exit();
		...
		if ($Mode == 'ADD_ITEM')
		{ 
			add_calibration_attachment($_POST['machine_id'], $_POST['trans_no'], $_POST['description'],
				$filename, $unique_name, $filesize, $filetype);
			display_notification(_("Attachment has been inserted.")); 
		}
		else
		{
			update_calibration_attachment($selected_id, $_POST['machine_id'], $_POST['trans_no'], $_POST['description'],
				$filename, $unique_name, $filesize, $filetype); 
...
if ($Mode == 'Delete')
{
	...
	delete_calibration_attachment($selected_id);	
...
function trans_view($trans)
{
	...
	return get_calibration_machine_maintenance_view_str(ST_CALIBRATION_INFO, $trans["trans_no"]);
}
...
function display_rows($type)
{
	...
	$sql = get_sql_for_machine_attached_documents($type);
...
{
	return "SELECT machine_id,description, filename, filesize, filetype, tran_date, id,trans_no FROM ".TB_PREF."calibration_attachment WHERE machine_id=".db_escape($machine_id)
